...
var ipAddress = 'http://192.168.0.101:8080/express-manage'
...
//获取密保问题下拉框
function getSelectAjax(id) {
    curId = id;
    //获取密保问题
    Ajax('get',ipAddress+'/user/getPasswordQuestionInfos',null,getQustions,null)
}
function getQustions(res) {
    var qustions = JSON.parse(res)
    var questionSelect = document.getElementById(curId);
    for (var i = 0 ; i <qustions.data.length; i ++) {
        var ele = document.createElement('option')
        ele.value = qustions.data[i].id;
        ele.innerText = qustions.data[i].questionName;
        questionSelect.appendChild(ele);
    }
